...
class ArrivalController extends VoyagerBaseController
{
    ...
    {
        if(Arrival::exists()){
        $arrival = Arrival::whereNull('Pox_id')->first();

        if($arrival){
            
            $Old_Box_Arrival=DB::table('arrivals')->whereNull('box_id')
            ->latest('created_at')->first();

            $New_Box_Arrival=DB::table('arrivals')->whereNotNull('box_id')
            ->latest('created_at')->first();

            // $br=Barccode::where('id',$New_Box_Arrival->Barcode_id)->first();


            Arrival::where('id',$New_Box_Arrival->id)->update
            ([
                'General_images'=> $Old_Box_Arrival->General_images,
                'Arrival_Images'=> $Old_Box_Arrival->Arrival_Images,
            ]);
        ...
        $Item_Windows=Packwindow::all();
        $count=Packwindow::exists();
    ...
    { 
        ...
        $userData = array
        (
            'box_id'=> $request->Pox_id, 
            'Pox_id'=> $request->Pox_id, 
